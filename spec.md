...
- **PROMISES STILL.** The `handles?` half of a message and rendezvous handle
  ...
  it from a spelling to THE primitive, with blocking send as a composition over
  it — and M4 unit 3 shipped every piece that composition needs while
  deliberately NOT shipping the composition as a library type (the ruling
  above). The ratified `send` SURFACE therefore arrives with unit 3.5's `Call`;
  what is proven today is that a caller can write it.
  **`PipeReply` IS STILL THE WRAPPER NAME AND NOT THE RESOLVED VALUE'S**: the
  ...
  this and asked the unit that built the value to name it). It is a WAIT-RECORD
  payload rather than a `send` return, which is what the ruling above leaves it
  as until `Call` gives it a second producer.
