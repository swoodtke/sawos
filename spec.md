...
**AND M4 UNIT 4's STAGED HANDLES ADD NO ROW AND NO REFERENCE CLASS TO THIS
TABLE, WHICH IS THE POINT OF THE RULE THEY IMPLEMENT** (sawos design 18;
`designs/010` ruling 5). A handle riding in a staged message is the SENDER's own
handle entry, still bound in the sender's table and already counted in the
"handle entries" column of whatever kind it names; what the ring slot holds is a
NOTE — a process index and a table word — and a note is not a reference. So
"in flight" is not a state any object can be in, no column had to learn about
messages, and the enumeration above is unchanged by a unit that moves
capabilities between processes. Two consequences the ledger gets for free: a
sender that RELEASES a staged entry drops the last reference on the ordinary
path, freeing the object if it was the last, and the receiver later finds an
EMPTY slot rather than a dangling one (the note is checked against the entry it
names, never trusted); and dropping a staged message at outlet death is
UNWIND-FREE, because there is nothing to give back to anybody. The transfer
happens at the rendezvous as an entry MOVE — unbind here, bind there, rights
verbatim — which is `Give`'s mechanism (§3) reached through a second door, and
a move changes no count at all.
...
- **BUILT M4 UNIT 4 — THE `handles?` HALF, AND THE SECTION HAS NO PROMISES LEFT**
  ...
  - **RENDEZVOUS TRANSFER IS RULING 5's LEDGER, AND IT HAS NO IN-FLIGHT STATE.**
    A staged handle stays the SENDER's counted entry — bound in its table,
    counted on its object's column, charged to its handle quota — and what the
    ring slot holds is a NOTE saying which table to look in. The entry MOVES at
    the take or the delivery, which is where the quota charge moves too. Three
    consequences, each proved: a sender may RELEASE a staged entry and the slot
    then arrives EMPTY (`tests/pipe-stale-attach`), which is what makes
    fire-and-forget honest; a taker whose table cannot accept the mints refuses
    ATOMICALLY, leaving the message staged and its level raised
    (`tests/pipe-table-full`); and the outlet-death staged drop is UNWIND-FREE,
    because the entries never left the sender and there is nothing to give back.
    **"No orphaned handles if the send is abandoned" now has something to be
    true of, and it is true.**
...
- **THE INVARIANT IS NO AMPLIFICATION — and it always was.** REWRITTEN
  ...
  - **A GIVE MOVES RIGHTS VERBATIM** (sawos design 4 D-1, M3 unit 3) —
    ...
    **AMENDED M4 UNIT 4 TO "VERBATIM, OR NARROWER"** (sawos design 18;
    ...
    as before.
    **AND RENDEZVOUS IS A THIRD DOOR ONTO THIS SAME MOVE** (design 18;
    ruling 5): a handle arriving in a message is unbound from the sender
    and bound in the receiver by the same mechanism, rights verbatim,
    inside the take or the delivery. What differs from `Give` is only
    WHEN — a give happens before the child starts, a rendezvous happens
    at any time between two running processes — and WHO consents: both
    ends, since one had to send and the other had to take.
...
- **Typed handles in the Saw API (ratified Aug 7, user).** Each object
  ...
  **TIER TWO IS BUILT** (sawos design 3 D-5, M3 unit 2.75; the Aug-29
  ...
  - **THE FUNNEL HAS A SECOND SPELLING NOW, FOR RECEIVERS** (SL-16, closed
    ...
    first consumers. TWO SHAPES KEEP THE SENTINEL, each for a reason:
    a funnel taking its wrapper as a by-value PARAMETER (`Process.give`,
    the four `Waiter.give` overloads), because `consumes` is a
    receiver-position effect; and `PipeReply.resolve`, because it consumes
    on two paths out of three and an effect that consumed on every path
    would spend a live claim at the first empty poll.
...
7. ~~Syscall ABI~~ **DECIDED (user, Jul 31; object-uniform Aug 5): (status,
   ...
   **AND M4 UNIT 4 RAN OUT OF REGISTERS AT LAST, WHICH IS WHY THE MESSAGE IS A
   ...
   - **AND THE VALIDATION HAPPENS BEFORE ANYTHING IS STAGED.** A send's handle
     words are resolved, checked for `Transfer`, and checked against the kinds a
     message may carry, all before the ring slot is touched — so a refused send
     leaves no half-built message, and a caller error still ends the caller by
     this item's faults ruling.
...
- **What the kernel does NOT have after M3.** One area per line, with the
  ...
  - **Quotas** (§12's creation-authority pin, which M2 ANSWERED with a
    ...
    **M4 UNIT 4 ADDED NO COLUMN EITHER AND MOVED ONE INSTEAD** (sawos design 18;
    `designs/010` ruling 5). A handle riding in a message is a `QuotaKind.Handle`
    row that already exists and stays the SENDER's while the message is staged —
    the entry never left its table — so a message in flight costs nothing new,
    and there is no third party to charge for a thing neither end holds. THE
    CHARGE MOVES AT THE RENDEZVOUS, in the same step as the table entry: the
    taker is charged and the sender credited when the take or the delivery
    copies the record, which is what makes "transfer" a bookkeeping identity
    rather than a mint plus a release. **AND IT IS THE TAKER'S BUDGET THAT
    REFUSES, ATOMICALLY**: a delivery mints up to `PIPE_MSG_HANDLES` + 1 entries
    at once, so the whole batch is pre-flighted against the taker's table AND its
    `Handle` row before anything is copied — a partial charge would leave a
    message half-delivered, which is the one state ruling 5 exists to make
    unrepresentable. A receiver at its budget is told `QuotaExceeded` and the
    message stays staged. Creator-pays is untouched by all of this: it decides
    who pays for an OBJECT, and rendezvous only ever moves who pays for a NAME.
  ...
  - **Pipes and PipeReplyHandle** (§2.1) — M4. **UNITS 1 AND 2 HAVE LANDED THE
    ...
    **AND UNIT 4 LANDED RENDEZVOUS** (sawos design 18), which finishes the
    ...
    the mechanism — a staged handle stays the SENDER's counted entry and the
    ring slot holds a NOTE, so there is no in-flight state, the ledger grew no
    reference class, a sender may release a staged entry and the slot arrives
    EMPTY, and the outlet-death drop stays unwind-free. The entry MOVES at the
    take or the delivery, rights verbatim, quota charge with it, and the batch
    is pre-flighted so a taker that cannot afford it refuses ATOMICALLY with the
    message left staged. Ruling 11(d)'s delivery-as-take arrived with it: a
    ...
    What remains is the driver-as-service money shot (unit 5).
