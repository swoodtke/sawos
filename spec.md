...
**BUILT SO FAR — M4 units 1, 2, 3 and 3.5: THE PAIR, THE ONE-SHOT PAIR,
WAITABILITY AND THE FUSED PATHS (sawos designs 13, 14, 15 and 17).** Nothing ratified above is reopened; what follows is a marker of
...
- **BUILT M4 UNIT 3.5 — THE FUSED PATHS, AND `send` IS A KERNEL CALL** (sawos
  ...
  - **WHAT IT COSTS, MEASURED**: `tests/pipe-pingpong` asserts ONE trap per RPC
    ...
    11(d)'s delivery-as-take lands in unit 4.
- **PROMISES STILL.** The `handles?` half of a message and rendezvous handle
  ...
  has nothing to be true of yet). The one-way `post` the client API paragraph
...
- **What the kernel does NOT have after M3.** One area per line, with the
  ...
  - **Pipes and PipeReplyHandle** (§2.1) — M4. **UNITS 1 AND 2 HAVE LANDED THE
    ...
    **AND UNIT 3.5 LANDED THE FUSED PATHS** (sawos design 17), which is where
    ...
    What remains is handles in messages (unit 4 — with ruling 11(d)'s
    delivery-as-take, which is what takes the server's two traps to one) and the
    driver-as-service money shot (unit 5).
