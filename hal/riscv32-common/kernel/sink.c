// SOS riscv32 KERNEL HAL, native half (designs 140, 172, 23).
//
// ONE COPY, SHARED BY EVERY riscv32 BOARD (design 23). This file was byte-
// identical between `hal/riscv32` and `hal/riscv32-esp32c3` — it names a CSR and
// two linker symbols and nothing about a board — so it lives here and both
// boards' build entries point at it. Nothing in it is board-conditional; if a
// board ever needs a different leaf, it declares that leaf itself (the `extern`
// block is already split by caller for exactly that reason — see
// `lib.saw`'s header).
//
...
//
// What used to be here and is now Saw: the console write loop and the stop (the
// BOARD's `lib.saw`), and all of the PMP region arithmetic — the config-word
// staging, the TOR pairing and the grant budget (`lib.saw` beside this file,
// the shared `rv32core` module). This file programs the registers it is handed
// and decides nothing.
//
// Each board's own `boot.S` is the trap entry and the M -> U transition, and
// its own linker script bounds the two symbol pairs below; `../user/syscall.c`
// is the process-side counterpart, and is shared the same way.
...
// after the kernel in the same ELF, in the `.payload` section the board's own
// linker script bounds with `_payload_start` / `_payload_end`. An image with no
// payload gets an empty section and start == end.
...
// (sawos design 2 D-2) lands in the `.regions` section the board's own linker
// script bounds; an image built with no children gets an empty section and
// start == end, which the kernel reads as ZERO REGIONS.
//
...
// budget spends, what a sosimg permission mask becomes — is `rv32core`'s
...
//
// C BECAUSE: the CSR number is an assembly-time immediate (reason 1 above).
// WHICH classes, and the shadow the mask is staged in, are the BOARD's — which
// is why this leaf is declared in each board's own `extern` block rather than
// in `rv32core`'s (design 23).
//
...
// resumes at the next instruction and the board's `irq_poll` asks the
// controller what arrived. A spurious wake costs one poll.
//
// A BOARD MAY NOT CALL IT: the ESP32-C3's `wait_for_irq` is deliberately empty
// (its emulator's interrupt matrix never raises `mip`, so `wfi` would sleep
// forever), so that board declares no `extern` for this leaf and `--gc-sections`
// drops it. The instruction is here for the boards — and the silicon — that do.
