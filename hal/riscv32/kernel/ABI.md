...
| `image_link_base(dest_base) -> UInt` | `dest_base`. **WHERE IT LOADS.** An image's link base and its destination frames are one number on a machine that does not translate, and a linker script that said otherwise would produce a program that runs nowhere. This is why this board keeps `root.ld`, `child.ld` and `child2.ld` while arm64 collapsed to one script (design 25 ruling 11 — identity is the tier-2 answer, kept as a living demonstration). |
