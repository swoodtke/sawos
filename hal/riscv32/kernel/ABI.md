...
| `phys_to_virt(pa) -> UInt` / `virt_to_phys(va) -> UInt` | **THE LINEAR-MAP SEAM, AND HERE IT IS THE IDENTITY** (sawos design 29; `rv32core`). The kernel's ruled discipline is that addresses-as-data stay PHYSICAL everywhere and only the moment of kernel DEREFERENCE converts — so `kernel/core` names these at every site where a stored physical address becomes a load or a store. This machine has no MMU: a physical address IS what the kernel loads through, both bodies return their argument, and every one of those calls folds away to nothing. The difference is ADVERTISED rather than faked (design 19) — Profile B's bodies are an OR and an AND with a fixed offset — and the kernel carries no tier conditional either way. **The punted Sv32 climb inherits it free** (design 25 ruling 11): this board's RAM at 0x8000_0000 already sits in the upper 2 GiB, so an Sv32 kernel never moves and the seam stays the identity even then. |
| `prot_reset()` | Revoke every grant. User mode then reaches nothing. |
| `map_source_ok(base, top) -> Bool` | **THE SAME BODY AS `map_target_ok`, AND THAT IS THIS PROFILE'S ANSWER TO PLACEMENT** (sawos design 33). The other profile split one question into two when it split one address into two — which INPUT addresses a domain may use, which OUTPUT frames it may reach. Here there is one address, so the two predicates are deliberately identical rather than one forwarding to the other, and each states its own question where a reader of either finds it. |
| `image_link_base(dest_base) -> UInt` | `dest_base`. **WHERE IT LOADS.** An image's link base and its destination frames are one number on a machine that does not translate, and a linker script that said otherwise would produce a program that runs nowhere. This is why this board keeps one script per resident image — `root.ld`, `child.ld`, `child2.ld` and `child3.ld` since design 36 — while arm64 collapsed to one script whatever the process count (design 25 ruling 11 — identity is the tier-2 answer, kept as a living demonstration). |
| `map_place(cursor, pa, device) -> UInt` | `pa`. The cursor is the translating tier's instrument and is ignored; the kernel keeps one per process on both tiers because it is arch-free bookkeeping. Design 25 ruling 1 holds here too: `map` takes no VA hint, the address is still the answer, and it happens to be an answer this machine has no freedom about. |
| `prot_region(idx, base, top, perms)` | Stage grant `idx` as `[base, top)` with `perms`, a mask in the `imgformat` `SegFlag` vocabulary (R = 1, W = 2, X = 4). Fed the row's PHYSICAL range by `load_domain`'s replay since design 33, which on this tier is also its user range — PMP filters addresses the processor actually issues. |
